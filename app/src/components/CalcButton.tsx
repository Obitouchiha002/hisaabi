...
export function CalcButton() {
  ...
  return (
    <>
      <button className="icon-btn" onClick={() => setOpen(true)} aria-label={t('Calculator', 'Calculator')}>
        {Icon.calc}
      </button>
...
function CalcSheet({ onClose }: { onClose(): void }) {
  ...
  return (
    <Sheet onClose={onClose}>
      <div className="section-title"><h2>{t('Calculator', 'Calculator')}</h2></div>
      ...
      <div className="calc-pad">
        ...
        <button className="calc-key" data-wide="" onClick={() => setText('')}>{t('Clear', 'Saaf')}</button>
        <button className="calc-key" onClick={() => setText((prev) => prev.slice(0, -1))}>⌫</button>
      </div>
